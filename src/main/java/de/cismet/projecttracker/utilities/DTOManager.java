...
/**
 * This class converts hibernate entities to DTOs with its clone methods and DTOs
 * to hibernate entities with its merge methods.
 *
 * @author therter
 */
public class DTOManager {
    ...
    public Object clone(BasicHibernateEntity original) throws InvalidInputValuesException {
        ...
        HashMap<Object, Object> convertedClasses = new HashMap<Object, Object>(INITIAL_CAPACITY);

        result = cloneInternal(original, convertedClasses);

        logger.debug("time to clone: " + ( System.currentTimeMillis() - startTime ) + "ms objects: " + convertedClasses.size());
        return result;
    }
    ...
    public ArrayList clone(List original) throws InvalidInputValuesException {
        long startTime = System.currentTimeMillis();
        ArrayList<BasicDTO> clonedList = new ArrayList<BasicDTO>();
        HashMap<Object, Object> convertedClasses = new HashMap<Object, Object>(INITIAL_CAPACITY);
        ...
        for (Object o : original) {
            if ( o instanceof BasicHibernateEntity ) {
                ...
                logger.error("Only objects of the type " + BasicHibernateEntity.class.getName() + " can be cloned. The given object is of the type " + o.getClass().getName());
                throw new InvalidInputValuesException("internal error");
            }
        }

        logger.debug("time to clone list: " + ( System.currentTimeMillis() - startTime ) + "ms objects: " + convertedClasses.size() );
        return clonedList;
    }
    ...
    public BasicHibernateEntity merge(BasicDTO original) throws InvalidInputValuesException {
        ...
        logger.debug("time to merge: " + ( System.currentTimeMillis() - startTime ) + "ms objects: " + convertedClasses.size() );
        return result;
    }
            ...
            if ( result == null && original != null ) {
                ...
                for (Method tmpMethod : methods) {
                    ...
                    if (name.startsWith("set")) {
                        try {
                            Method originalGetter = original.getClass().getMethod("g" + name.substring(1));
                            Class[] parameter = tmpMethod.getParameterTypes();

                            if ( !tmpMethod.isSynthetic() && parameter.length == 1 && originalGetter.getParameterTypes().length == 0) {
                                ...
                                if (BasicHibernateEntity.class.isAssignableFrom(declaredClass) && BasicDTO.class.isAssignableFrom( originalGetter.getReturnType() )) {
                                    tmpMethod.invoke( result, mergeInternal( (BasicDTO)originalGetter.invoke(original), convertedClasses) );
                                } else if ( Set.class.isAssignableFrom( declaredClass ) && ArrayList.class.isAssignableFrom( originalGetter.getReturnType() ) ) {
                                    Set newSet = new HashSet();
                                    ArrayList origList = (ArrayList)originalGetter.invoke(original);
                                ...
                                Class declaredClass = parameter[0];
                                 if (BasicDTO.class.isAssignableFrom(declaredClass) && BasicHibernateEntity.class.isAssignableFrom( originalGetter.getReturnType() )) {
                                     if (declaredClass.getName().endsWith("Short")) {
                                        Object para = cloneInternal( (BasicHibernateEntity)originalGetter.invoke(original), convertedClasses);
                                        tmpMethod.invoke( result, para.getClass().getMethod("toShortVersion").invoke(para) );
                                     } else {
                                        tmpMethod.invoke( result, cloneInternal( (BasicHibernateEntity)originalGetter.invoke(original), convertedClasses) );
                                     }
                                } else if ( ArrayList.class.isAssignableFrom( declaredClass ) && Set.class.isAssignableFrom( originalGetter.getReturnType() ) ) {
                                    ArrayList newSet = new ArrayList();
                                    Set origSet = (Set)originalGetter.invoke(original);
