...
//        form.center();
    ...
    private void addTime(final Date day) {
            ...
            protected void afterExecution(Boolean result, boolean operationFailed) {
                if (!operationFailed) {
                    if (!result || ProjectTrackerEntryPoint.getInstance().isAdmin()) {
                        List<TimeNotice> timeList = Story.this.taskMap.get(Story.this.daysOfWeek[day.getDay()]);
                        if (timeList.isEmpty()) {
        ...
        ProjectTrackerEntryPoint.getProjectService(true).isDayLocked(day, ProjectTrackerEntryPoint.getInstance().getStaff(), callback);
