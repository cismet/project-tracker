...
public class TaskStory extends Composite implements TaskDeleteListener, DoubleClickHandler, TaskNoticeListener {
    ...
    private FlowPanelWithSpacer draggableParent;
    ...
    private PickupDragController favController;
    private DragHandler demoDragHandler;
    ...
    public TaskStory(FavouriteTaskStory favs) {
        ...
        demoDragHandler = new DragHandler() {
            @Override
            ...
            public void onDragStart(DragStartEvent event) {
                //save information to restore the dragged widget
                draggableParent = (FlowPanelWithSpacer)((Widget)event.getSource()).getParent();
                draggableIndex = draggableParent.getWidgetIndex((Widget)event.getSource());
            }

            @Override
            public void onDragEnd(DragEndEvent event) {
                RestorePickupDragController o = (RestorePickupDragController)event.getContext().dragController;
                    ...
                    if (taskList != null && selectedWidgets != null) {
                        for (Widget tmp : selectedWidgets) {
                            if (tmp instanceof TaskNotice) {
                                ...
                                //restore the draggable widget
                                ((InsertPanel) draggableParent).insert(tmp, draggableIndex);
                            }
                        }
                    }
                }
...
//        sundayDragController = new RestorePickupDragController(RootPanel.get("contentId"), false);        
        mondayDragController = new RestorePickupDragController(this.boundaryPanel, false);
        tuesdayDragController = new RestorePickupDragController(this.boundaryPanel, false);
        wednesdayDragController = new RestorePickupDragController(this.boundaryPanel, false);
        thursdayDragController = new RestorePickupDragController(this.boundaryPanel, false);
        fridayDragController = new RestorePickupDragController(this.boundaryPanel, false);
        saturdayDragController = new RestorePickupDragController(this.boundaryPanel, false);
        sundayDragController = new RestorePickupDragController(this.boundaryPanel, false);        
        
        
//        favController = new RestorePickupDragController(favs.boundaryPanel, false);
//        initFavDragController();
    ...
    public void initDragController(PickupDragController controller, FlowPanelWithSpacer except) {
        ...
        controller.addDragHandler(demoDragHandler);
...
//        FlowPanelDropController widgetDropController = new FlowPanelDropController(favs.recentTasks);
//        controller.registerDropController(widgetDropController);
    }
    
    public void initFavDragController() {
        favController.setBehaviorMultipleSelection(false);
        favController.addDragHandler(demoDragHandler);

        FlowPanelDropController widgetDropController = new FlowPanelDropController(favs.recentTasks);
        favController.registerDropController(widgetDropController);
    }
    
    public DragHandler getDragHandler() {
        return demoDragHandler;
    }
...
//        favController.makeDraggable(widget, widget.getMouseHandledWidget());
